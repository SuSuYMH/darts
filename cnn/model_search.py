...
class MixedOp(nn.Module):
  def __init__(self, C, stride):
    ...
    for primitive in PRIMITIVES:  #PRIMITIVES中就是8个原子操作字符串，OPS[primitive]就转化成相应的操作了
  ...
  def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev):
    ...
    for i in range(self._steps):
      ...
      for j in range(2+i): #对第i个节点来说，他有j个前驱节点（每个节点的input都由前两个cell的输出和当前cell的前面的节点组成）
        ...
        op = MixedOp(C, stride) #op是构建两个节点之间的操作混合
...
class Network(nn.Module):
  ...
  def __init__(self, C, num_classes, layers, criterion, steps=4, multiplier=4, stem_multiplier=3):
    ...
    self._multiplier = multiplier # 好像是代表有几个结点的输出最后搞起来作为最后的输出，但这一般不就是steps这个数吗？？？
    ...
    for i in range(layers):  # 网络是8层，在1/3和2/3位置是reduction cell 其他是normal cell，reduction cell的stride是2
      if i in [layers // 3,
               2 * layers // 3]:  # 对应论文的Cells located at the 1/3 and 2/3 of the total depth of the network are reduction cells
        C_curr *= 2
        reduction = True
      else:
        reduction = False
      ...
      cell = Cell(steps, multiplier, C_prev_prev, C_prev, C_curr, reduction,
                  reduction_prev)  # 每个cell的input nodes是前前cell和前一个cell的输出
...
'''
  ...
  # 找到权重最大的两个节点及其对应的操作（darts默认对cnn找两个，对rnn找一个）
  def genotype(self):
    def _parse(weights):
      gene = []
      n = 2
